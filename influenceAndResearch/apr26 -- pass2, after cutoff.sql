...
delete #combine where ticketNum is null
update #combine set [requested Story Staff]=isnull([requested Story Staff],''), [final Story Staff]=isnull([final Story Staff],'')

update c
	set c.[Action Detail 3]=replace(replace(c.[Action Detail 3],'R. Lore: ',''),'Lore: ','')
	,c.[Action Detail 4]=replace(replace(c.[Action Detail 4],'R. Lore: ',''),'Lore: ','')
	from #combine c
