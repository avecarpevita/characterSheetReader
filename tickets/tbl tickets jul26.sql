...
use tm
/*
rebuilds the ENTIRE tickets table
*/

--select top 100 * from sys.tables order by create_date desc--rawCpData
--select * from eventsWithDates order by 2 desc
declare @eventName nvarchar(255)='Event 92 July 2026'
if not exists (select null from eventsWithDates ed where eventName='Event 92 July 2026')
	insert into eventsWithDates select 'Event 92 July 2026','2026.04.23'
...
truncate table #bi
bulk insert #bi from 'C:\characterSheetReader\tickets\apr26.tsv' with(datafiletype='char')
insert into #allRawTickets (eventName,eventDate,fn,ln,ticketType,rawPlayerName)
	select eventName,eventDate,fn,ln,ticketType,rawPlayerName from #bi b
		join eventsWithDates e on e.eventName='Event 92 July 2026'
...
eventName                                          
-------------------------------------------------- -----------
Event 92 July 2026                                193			--smells right
Event 89 February 2026                             198
...
select ticketType,count(*) from tickets where eventName='Event 92 July 2026'--536
