...
;with cte_charactersLast3Games as (select c.playerName,c.characterName
	...
	from rawCpData c
		...
		where eventName like '%event%'
			...
			or eventName like '%event 8[5678]%')
		)
select * 
...
update #activeCharacters set playerName=ltrim(rtrim(playerName)), characterName=ltrim(rtrim(characterName))
	...
	,eventDate=dbo.getEventDate(eventName,rawEventDate)
...
Adriana Beals			select top 100 * from tickets where eventName='Event 89 February 2026' and playerName like '%bea%'--not there			adrianakbeals@gmail.com
Graham Cummings			select top 100 * from tickets where eventName='Event 89 February 2026' and playerName like '%cumm%'--not there			cummingsgracee@gmail.com
Katt Jean				select top 100 * from tickets where eventName='Event 89 February 2026' and playerName like '%jean%'--as KJ
Lee Emig
Lee Emig
Morgan A Smith			select top 100 * from tickets where eventName='Event 89 February 2026' and playerName like '%morgan%'--as Morgan Smith
Rj Canonigo				select top 100 * from tickets where eventName='Event 89 February 2026' and playerName like '%canon%'--not there
Steven Calise			select top 100 * from tickets where eventName='Event 89 February 2026' and playerName like '%calis%'--it's there as Steve
William or Will			select top 100 * from tickets where eventName='Event 89 February 2026' and playerName like '%Will%'--probably william conti, Full Event: NPC Friday 8pm-12am

select top 100 * from tickets where eventName='Event 89 February 2026' and playerName like '%Warner%'



select top 100 * from anchorChangeLog where email='itscvclic@gmail.com'
select top 100 * from anchorChangeLog where timeslot like 'SatPMC%'