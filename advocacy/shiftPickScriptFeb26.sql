...
/*
2026.01.02 -- added carrying the discord handle
*/
...
bulk insert #shiftFormRaw from 'c:\characterSheetReader\advocacy\feb26.tsv' with(datafiletype='char',firstrow=2)
--select * From #shiftFormRaw where advocatename is null
delete #shiftFormRaw where advocatename is null
update #shiftFormRaw set advocateName=ltrim(rtrim(advocatename))
update #shiftFormRaw set advocateName=replace(advocatename,'GÇÖ','`')--JGÇÖAmy Pacheco
update #shiftFormRaw set discord=ltrim(rtrim(replace(discord,'@','')))
...
update #shiftFormRaw set preferredShift='SUN 8am-Noon [Logi Only]' where preferredShift='SUN 10am-2pm [Logi Only]'--removing this one entirely
...
--select * from #shiftFormRaw where advocatename like '%Sid%Dom%'
--select * from #shiftFormRaw
--select * from #shiftFormRaw where advocateName like '%jir%'
--#shiftFormRaw where advocateName like '%rose%'--
--#shiftFormRaw where advocateName like '%riss%'--
--#shiftFormRaw where preferredShift like '%SUN%'--
...
--select * from #shiftForm where advocateName like 'sid%'
--If Sidney Domholdt selects SUN 8am-Noon [Logi Only] as preferredShift, set their random number to 1000
update s	
	set rn=1000
	from #shiftForm s where advocateName like '%sid%dom%' and preferredShift='SUN 8am-Noon [Logi Only]'
...
--2026.01.02--added discord here (inline)
;with cte as (select distinct shiftOrder,[shift] from #shiftGrid)
select d.shiftOrder
	,d.[shift]
	,isnull(lt.advocate+' @'+(select top 1 discord from #shiftForm s where s.advocateName=lt.advocate),'') as LogiTrainee
	,isnull(lf.advocate+' @'+(select top 1 discord from #shiftForm s where s.advocateName=lf.advocate),'') LogiFull
	,isnull(st.advocate+' @'+(select top 1 discord from #shiftForm s where s.advocateName=st.advocate),'') StoryTrainee	
	,isnull(sf.advocate+' @'+(select top 1 discord from #shiftForm s where s.advocateName=sf.advocate),'') StoryFull
	from cte d
...
--unassigned
select advocateName+' @'+discord advocateName
	,logiStory,advocateLevel,preferredShift,blockedShift 
	from #shiftForm where assignedShift is null 
	and logiStory='Logi'
	order by newid()--
select advocateName+' @'+discord advocateName
	,logiStory,advocateLevel,preferredShift,blockedShift
